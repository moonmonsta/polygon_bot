...
export class TokenService {
  ...
  private balanceCache: Map<string, { balance: ethers.BigNumber, timestamp: number }> = new Map();
  private cacheValidityPeriod = 5 * 60 * 1000; // 5 minutes
  
  constructor(private multicallProvider: MulticallProvider) {
  ...
  public async loadTokens(tokenConfigs: any[]): Promise<void> {
    ...
    for (const [batchIndex, batch] of batches.entries()) {
      ...
      for (const tokenConfig of batch) {
        ...
        const tokenContract = new ethers.Contract(tokenConfig.address, IERC20ABI, this.multicallProvider);
  ...
  private getSpecialPairBoost(token0: string, token1: string): number {
    ...
    const isWell known = config.HIGH_LIQUIDITY_PAIRS.some(pair => 
  ...
  public async getTokenDecimals(address: string): Promise<number> {
    ...
    // Fetch decimals
    try {
      const tokenContract = new ethers.Contract(address, IERC20ABI, this.multicallProvider);
   ...
   */
  public async getTokenBalance(tokenAddress: string, walletAddress: string): Promise<ethers.BigNumber> {
    ...
    try {
      // Fetch balance
      const tokenContract = new ethers.Contract(tokenAddress, IERC20ABI, this.multicallProvider);
